<x-app-layout>
    <x-slot name="header"><h1 class="text-base font-semibold text-slate-800">Matériel</h1></x-slot>
    <x-slot name="headerActions">
        <a href="{{ route('admin.equipment.create') }}" class="inline-flex items-center gap-1.5 rounded-lg bg-amber-500 px-3 py-1.5 text-sm font-medium text-white shadow-sm hover:bg-amber-600 transition-colors">
        ...
        <table class="min-w-full text-sm">
            <thead class="bg-slate-50 border-b border-slate-200">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Quantité</th>
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex gap-2">
                                <a href="{{ route('admin.equipment.edit', $item) }}" class="inline-flex items-center rounded-md border border-slate-300 bg-white px-2.5 py-1 text-xs font-medium text-slate-700 hover:bg-slate-50 transition-colors">Modifier</a>
                                <form method="POST" action="{{ route('admin.equipment.destroy', $item) }}">@csrf @method('DELETE')
                                    <button onclick="return confirm('Supprimer ?')" class="inline-flex items-center rounded-md bg-red-50 px-2.5 py-1 text-xs font-medium text-red-700 ring-1 ring-red-200 hover:bg-red-100 transition-colors">Supprimer</button>
                                </form>
                            </div>
                        </td>
                    </tr>
